...
public class ConnectReqHandler extends SimpleChannelInboundHandler<Message> {
    ...
    public ConnectReqHandler(String device) {
        this.device = device;
    }
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        System.out.println("doAction--->"+this.device + "发起连接请求...");
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (message.getType() == MessageType.CONNECT_FAIL.getValue()) {
            System.out.println("doAction--->"+this.device + "连接失败...");
            ctx.close();
        } else if (message.getType() == MessageType.CONNECT_SUCCESS.getValue()) {//登录验证成功
            System.out.println("doAction--->"+this.device + "连接成功...");
