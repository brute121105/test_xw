...
public class DragImageUtil3 {
    ...
    public int  bakGetEndX(Bitmap bit) {
        pic1_rgb_r_min =230;
        pic1_rgb_r_max=250;
        int endX = getPic2LocX(bit,pic1X_1,pic1_rgb_r_min,pic1_rgb_r_max);
        if(endX==0) {
            pic1_rgb_r_min =230;
            pic1_rgb_r_max = 255;
    ...
    //获取方块一白色 像素点
    public List<PixPoint> getScanXPoints(Bitmap bi, int locPic1, int pic1_rgb_r_min, int pic1_rgb_r_max){
        List<PixPoint> pps = new ArrayList<PixPoint>();
        int i = minY;
        while(i>=minY&&i<=maxY) {
            int R = getImageRByXY(bi,locPic1,i);
    ...
    public int  getPic2LocX(Bitmap bi,int locPicX1,int pic1_rgb_r_min,int pic1_rgb_r_max) {
        //方块1坐落 像素点
        List<PixPoint> ps =  getScanXPoints(bi,locPicX1,pic1_rgb_r_min,pic1_rgb_r_max);
        if(ps.size()==0) {
            System.out.println("getScanXPoints size = 0, target X:0");
            return 0;
        }
        for(PixPoint p :ps) {
    ...
    public List<PixPoint> getPointsByScanY(Bitmap bi,int scanY){
        List<PixPoint> pps = new ArrayList<PixPoint>();
        int i = pic2_minX;
    ...
    //查找相邻两点像素差别最大的点
    public List<PixPoint> getSubtractPointBySubValue(Bitmap bi,int scanY,int subValue) {
        List<PixPoint> pps = getPointsByScanY(bi,scanY);
		/*for(PixPoint pp:pps) {
			System.out.println("getSubtractPointBySubValue pp-->"+JSON.toJSONString(pp));
		}*/
        List<PixPoint> result = new ArrayList<PixPoint>();
        //刷选相邻像素点差别较大的点
        int i=0;
    ...
    //获取坐标颜色RGB
    public int getImageRByXY(Bitmap bi,int x,int y) {
